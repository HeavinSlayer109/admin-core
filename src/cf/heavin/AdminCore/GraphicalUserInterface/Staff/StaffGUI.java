package cf.heavin.AdminCore.GraphicalUserInterface.Staff;
...
public class StaffGUI {
	...
	public StaffGUI(Player player) {
		Inventory gui = Bukkit.createInventory(null, 54, ChatColor.BLACK + "Staff GUI");
		@SuppressWarnings("deprecation")
		ItemStack guis = new ItemStack(Material.LEGACY_STAINED_GLASS_PANE, 1, (short) 4);
		...
		ItemStack settings = new ItemStack(Material.CLOCK);
		...
		evanishlore.add("");
		evanishlore.add(ChatColor.translateAlternateColorCodes('&', "&7Turning on &7vanish mode will &ehide"));
		...
		} else if (Methods.vanishCheck.containsKey(player.getName())) {
		...
		dvanishlore.add("");
		dvanishlore.add(ChatColor.translateAlternateColorCodes('&', "&7Turning off &7vanish mode will &eshow"));
		...
		if (!Methods.nvisionCheck.containsValue(player)) {
			...
			enightvisionMeta.setLore(envisionlore);
			enightvision.setItemMeta(enightvisionMeta);
			gui.setItem(13, enightvision);
		...
		settingsMeta.setDisplayName(Methods.color("&eSettings"));
		List<String> settingslore = new ArrayList<>();
		settingslore.add(Methods.color("&7Customize your settings to"));
		settingslore.add(Methods.color("&7your liking"));
		settingsMeta.setLore(settingslore);
		settings.setItemMeta(settingsMeta);
		gui.setItem(53, settings);
