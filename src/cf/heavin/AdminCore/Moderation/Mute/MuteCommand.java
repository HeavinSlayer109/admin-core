...
public class MuteCommand implements CommandExecutor{
	Reports muteConfig = AdminCore.getReportsConfig();
			...
			if (sender.hasPermission("admincore.report") || sender.hasPermission("admincore.*")) {
				if (args.length == 0) {
					...
					if (target == null) {
			          ...
			          int list;
			          if (muteConfig.getConfig().getConfigurationSection(target + ".") == null) {
			        	  list = 1;
			          } else list = muteConfig.getConfig().getConfigurationSection(target + ".").getKeys(false).size() + 1;
			          ...
			          for (Player ol : Bukkit.getOnlinePlayers()) {
			            ...
			            muteConfig.getConfig().set(target + "." + list + ".reporter", sender.getName());
			            muteConfig.getConfig().set(target + "." + list + ".reason", reason);
			            muteConfig.getConfig().set(target + "." + list + ".time", dtf.format(now));
			            muteConfig.save();
			        }
				}
			}
